...
class EarningPvController extends BaseController
{
    ...
    public static function EarningMinute()
    {
        ...
        while ( $data = json_decode(Redis::rpop("earning_pv"), true) ) 
        {
            //EarningPv::where('id', '=', $data['id'])->update(['state'=>2]);
            
            $this_time = substr($data['created_at'], 0, -4).'0:00';

            //EarningMinute 查找
            $earningMinute = EarningMinute::where('webmaster_id', '=', $data['webmaster_id'])
                ->where('myads_id', '=', $data['myads_id'])
            ...
            if(empty($earningMinute))
            {
                ...
                $earningMinute->webmaster_id = $data['webmaster_id'];
                $earningMinute->position_id = $data['position_id'];
                $earningMinute->myads_id = $data['myads_id'];
                $earningMinute->pv = 5;
            ...
            //AllianceHour 
            $this_time = substr($data['created_at'], 0, -6).':00:00';

            $allianceHour = AllianceHour::where('alliance_id', '=', $data['alliance_id'])
                ->where('time', '=', $this_time)
                ->first();
            ...
            if(empty($allianceHour))
            {
                ...
                $allianceHour->alliance_id = $data['alliance_id'];
            ...
            echo date("m-d H:i:s") . " PV结算位置：".$data['created_at']."\n";
            usleep(5000); //0.005秒
            //修改状态
            //EarningPv::where('id', '=', $data['id'])->update(['state'=>4]);
        }
